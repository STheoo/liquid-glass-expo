import type { SkImageFilter, SkRuntimeEffect, SkShader } from "@shopify/react-native-skia";
import {
  BackdropFilter,
  Canvas,
...
import { ReduceMotion, useDerivedValue, withSpring } from "react-native-reanimated";
...
export const Scene = ({ filter: filterCB, shader: shaderFilter }: SceneProps) => {
  ...
  const filter = useDerivedValue(() => {
    const localMatrix = processTransform2d([{ translateX: bounds.x }, { translateY: bounds.y }]);
    if (filterCB) {
      const shader = source.makeShader(processUniforms(source, uniforms.value), localMatrix);
